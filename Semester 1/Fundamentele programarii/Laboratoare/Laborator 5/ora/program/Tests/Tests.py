...
def testSearchExpense():
    ...
    assert searchExpensesGreaterThanSum(expenses,100) == {0 : e1, 1 : e2, 2 : e3}
    assert searchExpensesGreaterThanSum(expenses,150) == {}
    assert searchExpensesBeforeDayAndLessThanSum(expenses,3, 150) == {1 : e2, 2 : e3}
    assert searchExpensesBeforeDayAndLessThanSum(expenses,4, 2.5) == {}
    assert searchExpensesOfType(expenses, ExpenseType.TELEFON) == {0 : e1, 2 : e3}
...
def testFilterExpense():
    ...
    assert filterExpensesByType(expenses,ExpenseType.IMBRACAMINTE) == {0 : e1, 1 : e2, 2 : e3, 3 : e4}
    assert filterExpensesByType(expenses,ExpenseType.TELEFON) == {1 : e2, 3 : e4}
    assert filterExpensesLessThanSum(expenses,102) == {0 : e1, 1 : e2}
    assert filterExpensesLessThanSum(expenses,150) == {0 : e1}
    assert filterExpensesLessThanSum(expenses,2.4) == {0 : e1, 1 : e2, 2 : e3, 3 : e4}