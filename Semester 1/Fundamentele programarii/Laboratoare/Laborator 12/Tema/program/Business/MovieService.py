...
class MovieService:
    ...
    def addMovie(self, title, description, type):
        ...
        if self.alreadyExistsRecursive(len(self.getAllMovies()), self.getAllMovies(), movie):
            return 0
    ...
    def alreadyExistsRecursive(self, length, list, movie):
        """
        ...
        if length == 0:
            return False
        return (self.alreadyExistsRecursive(length-1, list, movie) or
                (list[length-1].getTitle() == movie.getTitle() and
                 list[length-1].getDescription() == movie.getDescription() and
                 list[length-1].getType() == movie.getType()))
