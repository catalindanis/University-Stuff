...
class ClientService:
    ...
    def __init__(self, repository):
        ...
        for client in self.__repository.getClients():
            if client.getId() > maxValue:
                maxValue = client.getId()
    ...
    def getClientByIdRecursive(self, length, list, client_id):
        """
        Functia returneaza un client dupa id-ul lui (in mod recursiv)
        :param length: lungimea listei de clienti (Integer)
        :param list: lista de clienti (List of Client)
        :param client_id: id-ul clientului (Integer)
        :return: clientul (Client) / None (daca nu exista clientul cu acel id)
        """
        if length == 0:
            return None
        result = self.getClientByIdRecursive(length-1, list, client_id)
        if list[length-1].getId() == client_id:
