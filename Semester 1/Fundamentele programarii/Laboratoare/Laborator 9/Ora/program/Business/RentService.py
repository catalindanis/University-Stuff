...
from Domain.Movie import MovieValidator, MovieType
...
class RentService:
    ...
    def getTopMovieTypesByNumberOfRents(self):
        """
        Functia returneaza dictionarul de genuri care contine in ordine descrescatoare
        genurile de filme si numarul de inchirieri
        :return: dictionarul cu cheia movie_type si valoare numarul de inchirieri
                (Dictionary)
        """
        initial = self.__repository.getAllMoviesClients().items()
        new_dict = {}
        ...
        for item in initial:
            if self.__movieService.idExists(item[0]):
                if self.__movieService.getMovieById(int(item[0])).getType().value not in new_dict:
                    new_dict[self.__movieService.getMovieById(int(item[0])).getType().value] = 0
                for id in item[1]:
                    if (self.__clientService.idExists(id)):
                        new_dict[self.__movieService.getMovieById(int(item[0])).getType().value] += 1
    ...
    def formatTypesDictShort(self, dict):
        """
        Functia formateaza dictionarul de genuri intr-un mesaj scurt pentru afisare
        :param dict: dictionarul cu cheia movie_type si valori numarul de inchirieri
                        (Dictionary)
        :return: mesajul formatat (String)
        """
        message = ""
        ...
        for type_id in dict:
            message += str(MovieType(type_id)).lower() + " " + (f"\tnumar de inchirieri: {dict[type_id]}\n")
