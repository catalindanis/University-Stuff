...
import repositories.InMemoryEventsRepository;

import java.util.List;

public class EventsService implements Service<Event> {
    private static EventsService instance;
    private final InMemoryEventsRepository repository;

    private EventsService() { repository = new InMemoryEventsRepository(); }
    ...
    public void subscribe(long userId, long eventId) {
        try {
            repository.get(eventId).subscribe(UsersService.getInstance().getById(userId));
        }
