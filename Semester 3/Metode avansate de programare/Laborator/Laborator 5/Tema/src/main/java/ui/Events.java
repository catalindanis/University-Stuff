...
class AddEvent extends AbstractUi {
    ...
    public void handle(String input) {
        switch(step) {
            ...
            case 2:
                try {
                    ...
                    if(parts.length != raceEventDTO.noLanes) throw new NumberFormatException();

                    raceEventDTO.lanes = new ArrayList<>();
                    for (String part : parts) {
                        int distance = Integer.parseInt(part.trim());
                        raceEventDTO.lanes.add(new Lane(distance));
                    }
                } catch (NumberFormatException e) {
        ...
        if(step == 3) {
            ...
            catch (EventException | GroupException e) {
...
class ExecuteEvent extends AbstractUi {
    ...
    public void handle(String input) {
        try {
            ...
            System.out.println("Ati introdus date invalide");
        } catch (NullPointerException | EventException e) {
