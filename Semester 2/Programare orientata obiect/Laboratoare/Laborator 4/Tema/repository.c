...
bool addExpenseInRepository(Expense* expense, ExpensesRepository* repository) {
	...
	if (repository->length == repository->capacity) {
		...
		Expense** temp = realloc(repository->expenses, repository->capacity * sizeof(Expense*));
		if (temp != NULL) 
			repository->expenses = temp;
	}
...
bool updateExpenseFromRepositoryByIndex(int index, Expense* newExpense, ExpensesRepository* repository) {
	...
	destroyExpense(repository->expenses[index]);
...
:param repository: referinta repository-ului (ExpensesRepository*)
*/
void destroyRepository(ExpensesRepository* repository) {
	for (int i = 0; i < repository->length; i++)
		destroyExpense(repository->expenses[i]);
	free(repository->expenses);
	free(repository);
}
