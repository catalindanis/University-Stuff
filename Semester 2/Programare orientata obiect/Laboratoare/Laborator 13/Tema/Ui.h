...
private:
	...
	QTableWidget* cartList = new QTableWidget{0, 4};
	...
	void initGUI() {
		...
		QStringList headers = { "Titlu", "Autor", "Genul", "An aparitie" };
		cartList->setHorizontalHeaderLabels(headers);
	...
	void initConnect() {
		QObject::connect(cartAddButton, &QPushButton::clicked, [&]() {
			...
			reloadCartList(this->cart.getAllBooks());
			});
	...
	void reloadCartList(const vector<Book>& cartBooks) {
		cartList->clear();

		QStringList headers = { "Titlu", "Autor", "Genul", "An aparitie" };
		cartList->setHorizontalHeaderLabels(headers);

		cartList->setRowCount(cartBooks.size());

		int currentRow = 0;
		for (const auto& book : cartBooks) {
			QTableWidgetItem* title = new QTableWidgetItem{};
			title->setText(QString::fromStdString(book.getTitle()));
			
			QTableWidgetItem* author = new QTableWidgetItem{};
			author->setText(QString::fromStdString(book.getAuthor()));

			QTableWidgetItem* type = new QTableWidgetItem{};
			type->setText(QString::fromStdString(book.getType()));

			QTableWidgetItem* releaseYear = new QTableWidgetItem{};
			releaseYear->setText(QString::number(book.getReleaseYear()));

			cartList->setItem(currentRow, 0, title);
			cartList->setItem(currentRow, 1, author);
			cartList->setItem(currentRow, 2, type);
			cartList->setItem(currentRow, 3, releaseYear);
			currentRow++;
		}
	}
public:
	...
	void update() override {
		reloadCartList(cart.getAllBooks());
	}
};
