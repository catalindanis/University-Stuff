...
private:
	...
	void initConnect() {
		...
		QObject::connect(deleteButton, &QPushButton::clicked, [&]() {
			if (table->selectedItems().isEmpty() == false) {
				this->service.removeBook(((Book*)table->currentItem()->data(Qt::UserRole).data())->getId());
				this->books = this->service.getAllBooks();
				reloadList();
		...
		QObject::connect(updateButton, &QPushButton::clicked, [&]() {
			if (!this->table->selectedItems().isEmpty()) {
				...
				int index = ((Book*) table->currentItem()->data(Qt::UserRole).data())->getId();
				try {
	...
	void reloadList() {		
		table->clear();
		table->setRowCount(this->books.size());
		table->setSelectionBehavior(QAbstractItemView::SelectRows);
		table->setSelectionMode(QAbstractItemView::SingleSelection);

		QStringList columns = { "Titlu", "Autor", "Tip", "Anul publicatiei" };
		table->setHorizontalHeaderLabels(columns);

		int currentRow = 0;
		for (const auto& book : books) {
			QTableWidgetItem* itemTitle = new QTableWidgetItem;
			QTableWidgetItem* itemAuthor = new QTableWidgetItem;
			QTableWidgetItem* itemType = new QTableWidgetItem;
			QTableWidgetItem* itemReleaseYear = new QTableWidgetItem;

			QVariant data;
			data.setValue(book);
			itemTitle->setData(Qt::UserRole, data);
			itemTitle->setText(QString::fromStdString(book.getTitle()));
			itemAuthor->setData(Qt::UserRole, data);
			itemAuthor->setText(QString::fromStdString(book.getAuthor()));
			itemType->setData(Qt::UserRole, data);
			itemType->setText(QString::fromStdString(book.getType()));
			itemReleaseYear->setData(Qt::UserRole, data);
			itemReleaseYear->setText(QString::number(book.getReleaseYear()));

			table->setItem(currentRow, 0, itemTitle);
			table->setItem(currentRow, 1, itemAuthor);
			table->setItem(currentRow, 2, itemType);
			table->setItem(currentRow, 3, itemReleaseYear);
			currentRow++;
		}
	}
