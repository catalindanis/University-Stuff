...
void CartService::generateCart(int size) {
	...
	std::uniform_int_distribution<> dist(0, (int)this->booksService.getAllBooks().size() - 1);
	while (size) {
		int rndNr = dist(mt);
		const Book& book = this->booksService.getBookByIndex(rndNr);
