...
public class ClientMain extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        try {
            ...
            if (inStream == null) {
                throw new RuntimeException("Cannot find app.properties in classpath at /ro/mpp2026/festivalmuzicajavafx/app.properties");
            }
            props.load(inStream);
        } catch (IOException e) {
        ...
        logger.debug("Connecting to server port {}", serverPort);
        logger.debug("Connecting to server host {}", serverHost);
        Navigator.setAuthService(new AuthServiceProxy(serverPort, serverHost));
        Navigator.setShowsService(new ShowsServiceProxy(serverPort, serverHost));
