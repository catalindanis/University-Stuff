...
public class MusicFestivalObjectWorker implements Runnable, Observer {
    ...
    public MusicFestivalObjectWorker(Socket connection, AuthService authService, ShowsService showsService) {
    ...
    private Response handleRequest(Request request) {
        if (request instanceof LoginRequest loginRequest) {
            ...
            try {
                User user = authService.login(userRequestDTO.email(), userRequestDTO.password(), this);
        ...
        if (request instanceof RegisterRequest registerRequest) {
            ...
            try {
                authService.register(userRequestDTO.email(), userRequestDTO.password());
        ...
        if(request instanceof SaveShowRequest saveShowRequest) {
            ...
            try {
                showsService.save(showRequestDTO.artistName(), showRequestDTO.date(), showRequestDTO.location(), showRequestDTO.remainingSeats());
                return new OkResponse();
            } catch (RuntimeException e) {
                connected=false;
                return new ErrorResponse(e.getMessage());
        ...
        if(request instanceof FindAllShowsRequest findAllShowsRequest) {
            ...
            try {
                List<ShowResponseDTO> shows = showMapper.convertToList(showsService.findAll(showFilter));
                logger.info(shows);
                OkResponse response = new OkResponse();
        ...
        if(request instanceof BookTicketRequest bookTicketRequest) {
            ...
            try {
                showsService.bookTicketForShow(
        ...
        if(request instanceof FindAllTicketsRequest findAllTicketsRequest) {
            ...
            try {
                List<TicketResponseDTO> tickets = ticketMapper.convertToList(showsService.findAllTickets());
                OkResponse response = new OkResponse();
        ...
        if(request instanceof FindTicketsForShowRequest findTicketsForShowRequest) {
            ...
            try {
                List<TicketResponseDTO> tickets = ticketMapper.convertToList(showsService.findAllTicketsForShow(showId));
                OkResponse response = new OkResponse();
                response.addData("tickets", tickets);
                return response;
            } catch (RuntimeException e) {
        ...
        if(request instanceof LogoutRequest logoutRequest) {
            ...
            try {
                authService.logout(userId, this);
