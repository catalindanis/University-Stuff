...
public partial class Auth : Form
{
    ...
    private void HandleLoginClick(object sender, EventArgs e)
    {
        ...
        {
            ValidateInputs();
            var user = AuthService.Instance.Login(emailField.Text, passwordField.Text);
            ResetFields();
            var props = new Dictionary<string, object> { { "user", user } };
            Navigator.NavigateTo(new Home(), "Dashboard", true, props);
        }
        catch (Exception ex)
        {
    ...
    private void HandleRegisterClick(object sender, EventArgs e)
    {
        ...
        {
            ValidateInputs();
            AuthService.Instance.Register(emailField.Text, passwordField.Text);
            ResetFields();
