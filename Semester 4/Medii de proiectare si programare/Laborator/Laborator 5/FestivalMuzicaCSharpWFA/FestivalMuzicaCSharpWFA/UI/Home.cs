...
public partial class Home : Form, IPropsReceiver, IObserver
{
    ...
    {
        var shows = ShowsService.Instance.FindAll();
        var showViewModels = shows.Select(s => new ShowViewModel(s, ShowsService.Instance.GetNumberOfSoldSeatsForShow(s.Id))).ToList();
    ...
    private void BuyButtonClick(object? sender, EventArgs e)
    {
        try
        {
            ...
            ShowsService.Instance.BookTicketForShow(showVm.Id, client, numberOfSeats);
            ResetFields();
            LoadData();
    ...
    {
        var ticketsForm = new ViewTickets();
        openedForms.Add(
    ...
    private void SearchButtonClick(object? sender, EventArgs e)
    {
        try
        {
            ...
            var filteredShowsForm = new FilteredShows();
            openedForms.Add(
