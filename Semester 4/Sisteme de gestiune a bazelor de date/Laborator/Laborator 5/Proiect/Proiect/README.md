...
## Audit trail pentru stergeri

Aplicatia salveaza acum fiecare stergere in tabela `audit_logs`.

Campurile principale sunt:

- `entity_name` - entitatea stearsa, de exemplu `Employee` sau `Department`
- `entity_id` - id-ul inregistrarii sterse
- `deleted_by` - numele utilizatorului Windows curent (`Environment.UserName`)
- `deleted_at` - momentul stergerii in UTC
- `delete_type` - `soft-delete` sau `hard-delete`

Tabela este creata automat prin migratia EF Core `AddAuditLogs`.
