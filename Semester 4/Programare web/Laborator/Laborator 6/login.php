                    ...
                    if ($stmt->rowCount() == 1) {
                        if ($user = $stmt->fetch()) {
                            if (password_verify($password, $user['password'])) {
                                ...
                                $stmt_log = $sqlite_pdo->prepare("INSERT INTO logs (action, email, status) VALUES ('login', ?, 'success')");
                                $stmt_log->execute([$email]);
                                
                                header("location: index.php");
                                exit;
                            } else {
                                $stmt_log = $sqlite_pdo->prepare("INSERT INTO logs (action, email, status) VALUES ('login', ?, 'failed')");
                                $stmt_log->execute([$email]);
